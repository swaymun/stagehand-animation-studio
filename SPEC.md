...
The current cut is a deterministic, local-first Paper Cutout Comedy scene: two pre-rigged characters with replaceable imported art, a diner background, captions, interpolated character and camera keyframe timelines, structured non-voice music/footstep/sting cues rendered into WebM, a structured asset library with placeholder add/remove and local image import/compositing, four reusable scene templates, command-backed transforms, poses, and reaction cuts, undo/redo, local recovery, independent multi-scene content with add/rename/duplicate/delete operations, storyboard-beat promotion into trimmed scenes, JSON import/export, visible readiness validation, and thirty-one WebMCP tools. TTS, voice cloning, dialogue recording, phoneme extraction, and lip-sync remain explicitly out of scope.
...
3. Multi-scene animation core — complete for the current cut: independent scene content, scene operations, interpolated camera framing, and storyboard-beat promotion into trimmed scenes are verified; richer storyboard authoring remains.
4. Agent-native control — in progress: thirty-one registered tools, revisions, undo, structured validation, and visible/agent parity are present; hosted live WebMCP enumeration is still a platform/browser capability gap.
5. Mixed media/templates — in progress: structured asset add/remove, local image import, deterministic character/background/prop compositing, character-art binding, and four reusable scene templates are present; asset briefs, character-sheet pose extraction, and an expanded style bible remain.
6. Captions/audio/validation/render — partial: captions, deterministic validation, cue-based music/SFX, and audio-capable WebM export are present; richer render controls remain.
