...
The current cut is a deterministic, local-first Paper Cutout Comedy scene: two pre-rigged characters with replaceable imported art and four-pose sheet crops, a diner background, captions, interpolated character and camera keyframe timelines, structured non-voice music/footstep/sting cues rendered into WebM, a structured asset library with placeholder add/remove and local image import/compositing, four reusable scene templates, command-backed transforms, poses, and reaction cuts, undo/redo, local recovery, independent multi-scene content with add/rename/duplicate/delete/reorder operations, storyboard-beat promotion into trimmed scenes, persisted 12/24 fps and 720p/1080p render settings, JSON import/export, visible readiness validation, and thirty-three WebMCP tools. TTS, voice cloning, dialogue recording, phoneme extraction, and lip-sync remain explicitly out of scope.
...
4. Agent-native control — in progress: thirty-three registered tools, revisions, undo, structured validation, and visible/agent parity are present; hosted live WebMCP enumeration is still a platform/browser capability gap.
