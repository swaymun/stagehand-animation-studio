...
8. Skeletal character rigs — future: accept a character reference image or pose sheet, propose an editable bone graph and art binding, require human review of joints and weights, then drive deformable character animation through the existing command, timeline, Preview, validation, and WebM export paths. This is intentionally separate from the current six-pose rig so the MVP remains deterministic and the proposal can fall back to the existing rigid-pose workflow when a reference cannot be safely interpreted.
