...
4. Agent-native control — in progress: forty-seven registered tools, revisions, undo/redo, structured validation, deterministic frame inspection, persistent character-track locking, split-scene parity, editable storyboard parity, editable style-bible parity, editable asset-brief parity, editable project-name parity, pose and motion-preset parity, audio-cue level/timing parity, current-frame export parity, agent-triggered project WebM rendering, and visible/agent parity are present; hosted live WebMCP enumeration is still a platform/browser capability gap.
