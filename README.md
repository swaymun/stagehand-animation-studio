...
5. `document.modelContext.registerTool()` registers the imperative WebMCP surface when the host provides it, with a `navigator.modelContext` compatibility fallback for current experimental Chromium builds.
